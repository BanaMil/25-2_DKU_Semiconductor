...
static unsigned int hostWriteCounter = 0; // Debug counters
...
void InitBlockMap()
{
	...
	for(dieNo=0 ; dieNo<USER_DIES ; dieNo++)
	{
		for(virtualBlockNo=0; virtualBlockNo<USER_BLOCKS_PER_DIE ; virtualBlockNo++)
		{
			...
			virtualBlockMapPtr->block[dieNo][virtualBlockNo].writeCnt = 0; // writeCnt Init
...
unsigned int AddrTransWrite(unsigned int logicalSliceAddr)
{
	...
	if(logicalSliceAddr < SLICES_PER_SSD)
	{
		...
		// execute writeCnt++
        {
            ...
            unsigned int blockNo = Vsa2VblockTranslation(virtualSliceAddr);

            virtualBlockMapPtr->block[dieNo][blockNo].writeCnt++;
        }
		...
		// Debug counter++
		hostWriteCounter++;
    	if (hostWriteCounter % 3000 == 0)
        	PrintBlockWriteCount();
...
/*********************************************************************
 *  Debug: Print Write Count of Each Block
 *********************************************************************/
void PrintBlockWriteCount(void)
{
    unsigned int dieNo, blockNo;

    xil_printf("\r\n==== [WL DEBUG] Block Write Count ====\r\n");
    ...
    for (dieNo = 0; dieNo < USER_DIES; dieNo++)
    {
        ...
        for (blockNo = 0; blockNo < USER_BLOCKS_PER_DIE; blockNo++)
        {
            VIRTUAL_BLOCK_ENTRY* blk = &virtualBlockMapPtr->block[dieNo][blockNo];

            if (blk->writeCnt > 0)
            {
                xil_printf("   blk=%4u  writeCnt=%8u  eraseCnt=%5u  invalid=%5u\r\n",
                           blockNo,
                           blk->writeCnt,
                           blk->eraseCnt,
                           blk->invalidSliceCnt);
